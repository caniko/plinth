{
  ...
  cfg = config.programs.plinth.siteChecks;
...
in {
  options.programs.plinth.siteChecks = {
    enable = lib.mkEnableOption "Plinth site check registry";

    package = lib.mkOption {
      type = lib.types.package;
      default = pkgs.plinth-cli or (throw "plinth-cli package not found. Set programs.plinth.siteChecks.package.");
    ...
    assertions = [
      {
        ...
        message = "programs.plinth.siteChecks.targets must not be empty when site checks are enabled.";
      }
    ];

    home.packages = [wrappedPackage];
    home.sessionVariables.PLINTH_SITE_CHECK_CONFIG = toString configFile;
    xdg.configFile."plinth/site-checks.toml".source = configFile;
  };
}
