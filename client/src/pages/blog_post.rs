...
#[cfg(feature = "ssr")]
use server::server_fns::get_blog_post_by_slug;
...
pub fn BlogPostPage() -> impl IntoView {
    ...
    // Load blog post via server function
    let blog_post = create_resource(
        slug,
        |slug| async move {
            #[cfg(feature = "ssr")]
            { get_blog_post_by_slug(slug).await }

            #[cfg(not(feature = "ssr"))]
            { server::server_fns::get_blog_post_by_slug(slug).await }
        },
    );
