...
pub async fn seed_sample_data(db: &Surreal<Db>) -> Result<(), surrealdb::Error> {
    ...
    let existing_posts: Vec<Thing> = db.query("SELECT id FROM blog_posts LIMIT 1").await?.take(0)?;
...
/// Helper to convert SurrealDB Thing to string ID
pub fn thing_to_string(thing: &Thing) -> String {
    format!("{}:{}", thing.tb, thing.id)
}

/// Helper to parse string ID to Thing
pub fn string_to_thing(id: &str) -> Result<Thing, String> {
    ...
    Ok(Thing {
        ...
        id: surrealdb::sql::Id::String(parts[1].to_string()),
    })
}
...
mod tests {
    ...
    #[test]
    fn test_thing_conversion() {
        let thing = Thing {
            tb: "blog_posts".to_string(),
            id: surrealdb::sql::Id::String("my-post".to_string()),
        };

        let id_string = thing_to_string(&thing);
        assert_eq!(id_string, "blog_posts:my-post");

        let parsed = string_to_thing(&id_string).unwrap();
        assert_eq!(parsed.tb, "blog_posts");
    }
}
