...
use crate::{
    AppState,
    services::markdown_processor::{parse_markdown, generate_slug},
...
pub async fn publish_article(
    ...
    let description = request.description
        .or_else(|| parsed.frontmatter.as_ref().and_then(|fm| fm.description.clone()))
        .unwrap_or_else(|| {
    ...
    let created_post = created.into_iter().next().ok_or_else(|| ErrorResponse {
        error: "No blog post returned from database".to_string(),
        details: None,
    })?;

    // Invalidate content cache
    let _ = state.content_cache
        .ask(InvalidateCache)
        .send()
        .await;
...
pub async fn delete_article(
    ...
    let _: Option<BlogPost> = db.delete(("blog_posts", post_id)).await
        .map_err(|e| ErrorResponse {
