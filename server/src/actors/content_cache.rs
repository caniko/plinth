...
use shared::{BlogPost, BlogListItem, PortfolioItem};
...
impl Message<GetBlogPost> for ContentCache {
    ...
    async fn handle(&mut self, msg: GetBlogPost, _ctx: Context<'_, Self, Self::Reply>) -> Self::Reply {
        // Check cache first
        if let Some(post) = self.blog_posts.get(&msg.0) {
            return Ok(Some(post.clone()));
        }

        // Query SurrealDB
        let result: Result<Option<BlogPost>, _> = self.db
        ...
        match result {
            Ok(Some(post)) => {
                // Update cache
                self.blog_posts.insert(msg.0, post.clone());
                Ok(Some(post))
            }
            Ok(None) => Ok(None),
...
impl Message<GetAllBlogPosts> for ContentCache {
    ...
    async fn handle(&mut self, _msg: GetAllBlogPosts, _ctx: Context<'_, Self, Self::Reply>) -> Self::Reply {
        // Check cache first
...
impl Message<GetPostsByTag> for ContentCache {
    ...
    async fn handle(&mut self, msg: GetPostsByTag, _ctx: Context<'_, Self, Self::Reply>) -> Self::Reply {
...
impl Message<GetPortfolioItem> for ContentCache {
    ...
    async fn handle(&mut self, msg: GetPortfolioItem, _ctx: Context<'_, Self, Self::Reply>) -> Self::Reply {
        // Check cache first
        if let Some(item) = self.portfolio_items.get(&msg.0) {
            return Ok(Some(item.clone()));
        }

        // Query SurrealDB
        let result: Result<Option<PortfolioItem>, _> = self.db
        ...
        match result {
            Ok(Some(item)) => {
                // Update cache
                self.portfolio_items.insert(msg.0, item.clone());
                Ok(Some(item))
            }
            Ok(None) => Ok(None),
            Err(e) => Err(format!("Database error: {}", e)),
...
impl Message<GetAllPortfolioItems> for ContentCache {
    ...
    async fn handle(&mut self, _msg: GetAllPortfolioItems, _ctx: Context<'_, Self, Self::Reply>) -> Self::Reply {
        // Check cache first
        ...
        // Query SurrealDB
        let result: Result<Vec<PortfolioItem>, _> = self.db
...
impl Message<InvalidateCache> for ContentCache {
    ...
    async fn handle(&mut self, _msg: InvalidateCache, _ctx: Context<'_, Self, Self::Reply>) -> Self::Reply {
