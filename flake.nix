{
  ...
  inputs = {
    ...
    nix-cache-pin.inputs.rs-harbor.follows = "rs-harbor";
    ...
    nix-pklx = {
      url = "git+ssh://git@codeberg.org/caniko/nix-pklx.git";
      inputs.nixpkgs.follows = "nixpkgs";
      inputs.crane.follows = "crane";
      inputs.rust-overlay.follows = "rust-overlay";
      inputs.rs-harbor.follows = "rs-harbor";
      inputs.plinth.inputs.nix-cache-pin.inputs.rs-harbor.follows = "rs-harbor";
      inputs.plinth.inputs.nix-pklx.inputs.rs-harbor.follows = "rs-harbor";
    };
  ...
  outputs =
    ...
    let
      ...
      cargoPackageVersion = name: let
        matches = nixpkgs.lib.filter (package: package.name == name) cargoLockSpec.package;
      in
        if matches == []
    ...
    in
      inputs.flake-parts.lib.mkFlake {inherit inputs;} {
        ...
        # via `plinth.lib.${system}.mkProjectSite` and `.mkDeployPagesApp`.
        transposition.lib = {
          adHoc = true;
        };
        ...
        flake = {
          ...
          overlays.default = final: prev: {
            inherit
              ...
              plinth-minimal
              ;
            plinth-dioxus-helper = inputs.self.packages.${final.stdenv.hostPlatform.system}.plinth-dioxus-helper;
          };
          crossPackages."x86_64-linux"."aarch64-linux".plinth = inputs.self.packages."x86_64-linux"."plinth-aarch64-linux";
        };
        ...
        perSystem = {
              ...
              if [ ! -f "$binaryPathForInstall" ]; then
                for candidate in target/*/release/plinth-web; do
                  if [ -f "$candidate" ]; then
                    ...
                    cliBinaryPath="$(dirname "$candidate")/plinth"
                    break
                  fi
                done
              fi
          ...
          siteChecksModuleMarkers = let
            ...
            homeEval = lib.evalModules {
              ...
              modules = [
                stubHomeOptions
                inputs.self.homeModules.site-checks
                {
            ...
            nixosEval = lib.evalModules {
              ...
              modules = [
                stubNixosOptions
                inputs.self.nixosModules.site-checks
                {
        ...
        in
          ...
          apps.default = {
            type = "app";
            program = "${plinth}/bin/plinth-server";
          };
