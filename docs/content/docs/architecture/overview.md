+++
title = "Overview"
description = "Plinth workspace architecture and data flow"
weight = 10
+++
...
Domain types used across the stack: `BlogPost`, `BlogListItem`, `PortfolioItem`, `SiteConfig`, `Tag`, `SiteContent`, `ContentFormat`, `PublishArticleRequest`. Also contains `serde_helpers` for SurrealDB `Thing` deserialization.
...
- **Services** (`services/`): SurrealDB operations, markdown processing
...
`AppState` holds `LeptosOptions`, actor refs (`ContentCache`, `VectorSearch`), the SurrealDB handle, HTTP client, and config.
...
3. Leptos server functions fetch data from the `ContentCache` actor (which reads from SurrealDB)
...
5. Server stores in SurrealDB, creates tag graph relations, invalidates `ContentCache`
6. `VectorSearch` actor picks up the new embedding on next cache refresh
