...
removes the two predecessor planning directories.

## Goal

The rendering architecture is documented in stable docs as a single source of truth,
and the two prior plans this set completes
...
3. **Confirm prior-plan durable knowledge is already in stable docs:**
   - the forge activity plan: `docs/src/api/activity.md`, `docs/src/guides/activity.md`,
     and the `[ranking]`/`[forge]`/`[feeds]` config docs already exist (shipped by
     the forge activity docs phase). Verify, don't rewrite.
   - the Postgres migration plan: the durable outcome (Postgres + pgvector + sqlx stack,
...
4. **Retire the prior plans:**
   - Remove their sections from `docs/src/SUMMARY.md` (the predecessor plan blocks, lines ~44–66 of the current file).
   - Remove the two predecessor planning directories
     (check `git ls-files` first; if any are untracked, `rm -rf` the remainder — see
     the retire-docs-planning lesson on tracking state).
5. **Stale-reference sweep.** `rg -n 'predecessor plan stale-reference patterns'`
...
- [ ] `rg` for predecessor-plan slugs in `docs/src/SUMMARY.md` returns zero hits.
- [ ] `rg -n 'predecessor plan stale-reference patterns' docs/src/`
      ...
      plan itself retires).
- [ ] predecessor-plan durable knowledge confirmed present in stable
...
- `docs/src/development/*` (only if folding a Postgres migration maintainer note not
  already covered).
- Deleted: the two predecessor planning directories.
...
- Retired predecessor plan sources were removed after their durable knowledge was verified in stable docs.
